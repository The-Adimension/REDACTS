...
export async function login(
  ...
  // After fresh install, REDCap may show a "Set Your Password" page
  // before allowing login. Handle that first-run step here.
  const setPasswordHeading = page.locator('h4:has-text("Set Your Password")');
  if (await setPasswordHeading.isVisible().catch(() => false)) {
    // Fields may be type="password" or type="text" - target all inputs in the table
    const pwFields = page.locator('table input[type="password"], table input[type="text"]');
    ...
    // After setting password, navigate fresh to get the login page
    await page.goto("/redcap/");
    await page.waitForLoadState("domcontentloaded");
  }

  // If we're already past login (auth_meth_global=none auto-authenticates as
  // site_admin, or password-set redirected us), skip form fill.
  const logoutLink = page.locator('a:has-text("Log out"), a:has-text("Logout")');
  if (await logoutLink.first().isVisible().catch(() => false)) return;
  ...
  // Wait for post-login page - confirm login by checking for the Logout link
  // (URL patterns vary across REDCap versions, so an element check survives the variation)
  await page.waitForLoadState("networkidle");
  await expect(
    page.locator('a:has-text("Log out"), a:has-text("Logout")')
      .first()
