  ...
  test("CSV export contains no injected code", async ({ page }) => {
    ...
    if (await exportBtn.isVisible({ timeout: 5_000 }).catch(() => false)) {
      ...
      const downloadRespPromise = page.waitForResponse(
        resp => (resp.headers()["content-disposition"] || "").includes("attachment"),
        { timeout: 30_000 }
      );
      ...
      try {
        ...
        // SEC076: Validate download response content-type and no injected code
        const downloadResponse = await downloadRespPromise;
        await assertCleanDownload(downloadResponse, "text/csv");
      } catch (err) {
    ...
    if (await addRecordBtn.isVisible({ timeout: 5_000 }).catch(() => false)) {
      ...
      if (await textField.isVisible({ timeout: 3_000 }).catch(() => false)) {
        ...
        if (await saveBtn.isVisible({ timeout: 3_000 }).catch(() => false)) {
          ...
          // SEC010/SEC073: Assert no reflected XSS using canonical assertion
          await assertNoReflectedXSS(page, xssPayload);
        }
      }
    } else {
