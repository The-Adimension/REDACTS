...
test.describe("Admin Access Workflow", () => {
  ...
  test("User Rights page loads without PHP errors", async ({ page }) => {
    ...
    const userTable = page.locator("table, #user-rights-table, .userRightsTable").first();
    ...
    // SEC010: Verify no reflected XSS in user rights display
  ...
  test("admin can access superuser pages without errors", async ({ page }) => {
    // Login as admin and verify superuser-only page loads cleanly
    await login(page);
