...
from threat_base.cwe_database import (
    CWE_ATTRIBUTION,
    CWE_CSV_FILENAME,
    CWE_VERSION,
    CweDatabase,
...
class TestOfflineGuarantee:
    ...
    def test_no_network_imports(self) -> None:
        source_path = (
            Path(__file__).resolve().parent.parent.parent / "threat_base" / "cwe_database.py"
        )
...
class TestBackwardCompatibility:
    ...
    def test_investigation_finding_new_fields(self) -> None:
        ...
        from static.analyze.step_protocol import InvestigationFinding
    ...
    def test_unified_finding_cwe_name_in_sarif(self) -> None:
        ...
        assert len(taxa) == 1
        assert taxa[0]["id"] == "89"
        assert taxa[0]["properties"]["name"] == "SQL Injection"
    ...
    def test_unified_finding_cwe_name_omitted_when_empty(self) -> None:
        ...
        assert "properties" not in taxa[0]
...
class TestBundledData:
    ...
    def bundled_db(self) -> CweDatabase:
        ...
        real_data_dir = Path(__file__).resolve().parent.parent.parent / "threat_base" / "data"
