...
and parity with an inline reference implementation.
"""
...
from static.scanners import sarif as sarif_utils
...
# --- 3. extract_sarif_results ---


class TestExtractSarifResults:
    """Canonical SARIF result extraction."""
...
class TestCountBySeverity:
    ...
    def test_invalid_finding_uses_fallback(self) -> None:
        ...
        class NoSeverity:
            """Object without a severity attribute."""
...
# --- 11. Reference-implementation parity ----------


class TestReferenceParity:
    """Pin canonical helpers to an inline reference implementation.

    Each test computes the expected value with a small inline algorithm,
    then asserts the production helper agrees. This guards against silent
    drift in the helper's flattening, counting, or extraction logic.
    """

    def test_extract_results_flattens_runs(self) -> None:
        """`extract_sarif_results` returns the concatenation of every run's results."""
        sarif = _make_sarif(num_results=5, num_runs=2)

        expected: list[dict[str, Any]] = []
        for run in sarif.get("runs", []):
            expected.extend(run.get("results", []))

        assert sarif_utils.extract_sarif_results(sarif) == expected

    def test_extract_results_single_run(self) -> None:
        """Flattening is stable for a single-run SARIF document."""
    ...
    def test_count_by_severity_groups_findings(self) -> None:
        """`count_by_severity` returns a tally keyed by severity value."""
        findings = [
            _FakeFinding("HIGH"),
            _FakeFinding("HIGH"),
            _FakeFinding("MEDIUM"),
        ...
        assert sarif_utils.count_by_severity(findings) == expected

    def test_extract_rules_returns_id_name_description(self) -> None:
        """`extract_rules` projects each driver rule to (id, name, shortDescription)."""
        sarif = _make_sarif(num_results=3, include_rules=True)
        ...
        assert sarif_utils.extract_rules(sarif) == expected

    def test_count_files_scanned_dedupes_uris(self) -> None:
        """`count_files_scanned` counts unique URIs across artifacts and result locations."""
        ...
        assert sarif_utils.count_files_scanned(sarif) == len(files)

    def test_extract_cwe_prefers_taxa_over_properties(self) -> None:
        """`extract_cwe` resolves CWE from the taxa block first, falling back to properties.cwe."""
        result_taxa = {
            "taxa": [{"id": "89", "toolComponent": {"name": "CWE"}}],
            "properties": {"cwe": "CWE-79"},
        }
        ...
        for taxa in result_taxa.get("taxa", []):
            ...
            if component.upper() == "CWE":
                expected = f"CWE-{taxa.get('id', '')}"
                break
        if not expected:
            expected = result_taxa.get("properties", {}).get("cwe", "")

        assert sarif_utils.extract_cwe(result_taxa) == expected

    def test_extract_execution_errors_collects_notification_text(self) -> None:
        """`extract_execution_errors` returns every non-empty toolExecutionNotification message."""
        sarif = _make_sarif(num_results=1, include_invocations=True)
        ...
        assert sarif_utils.extract_execution_errors(sarif) == expected
