...
from static.scanners.phase_protocol import (
    OrchestratorContext,
    PhaseResult,
)
from static.scanners.orchestrator import (
    OrchestratorConfig,
    ToolOrchestrator,
)
...
def test_custom_phases_via_constructor(tmp_path: Path) -> None:
    """Test that custom phases can be injected via the constructor."""
    phases = [
        MockPhase("first"),
        MockPhase("second"),
        MockPhase("third"),
    ]
    orchestrator = ToolOrchestrator(target_path=tmp_path, phases=phases)
