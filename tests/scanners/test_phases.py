"""Tests for orchestration.phases.regex_scanner_phase.
...
from static.analyze.scanner import SecurityFinding, SecurityReport
from static.scanners.phase_protocol import OrchestratorContext
from static.scanners.phases import RegexScannerPhase
...
def mock_config():
    ...
    config = MagicMock()
    config.enable_regex_scanner = True
    return config
...
def test_name(phase):
    ...
    assert phase.name == "regex_scanner"


def test_execute_skipped_when_disabled(phase, mock_context):
    """Phase skips itself if config disables the regex scanner."""
    mock_context.config.enable_regex_scanner = False
...
@patch("static.analyze.scanner.SecurityScanner")
@patch("static.scanners.phases.regex_scanner_phase.normalize_security_finding")
...
def test_execute_exception_handling(mock_scanner_class, phase, mock_context, caplog):
    ...
    assert result.skipped is False
    assert len(mock_context.collection.findings) == 0
    assert "Regex scanner phase failed" in caplog.text
