...
from static.core import constants
...
class TestSeverityCvss:
    ...
    def test_critical_representative(self) -> None:
        """`SEVERITY_CVSS['CRITICAL']` is the expected 9.5 midpoint."""
...
class TestScannableExtensions:
    """Extensions eligible for content scanning."""
...
class TestThresholds:
    """Entropy and binary-detection threshold constants."""
...
# --- 10. Reference parity - verify canonical values cover every required entry ---


class TestReferenceParity:
    """Ensure the canonical constants are exact supersets of every required map."""

    def test_language_map_covers_core_extensions(self) -> None:
        """`get_language_map` resolves the 23 core source/markup/data extensions."""
        expected = {
    ...
    def test_language_map_is_consistent_across_callers(self) -> None:
        """The same 23 extensions resolve identically (no per-caller drift)."""
        expected = {
            ".php": "PHP", ".js": "JavaScript", ".css": "CSS",
            ".html": "HTML", ".htm": "HTML", ".xml": "XML",
            ".json": "JSON", ".sql": "SQL", ".py": "Python",
    ...
    def test_category_map_covers_source_categories(self) -> None:
        """`get_category_map` is a superset of the source/markup/style/data/config/doc map."""
        expected = {
        ...
        for cat, exts in expected.items():
            assert exts <= canonical[cat], f"Category {cat!r} missing entries"

    def test_category_map_covers_binary_category(self) -> None:
        """`get_category_map` is a superset of the manifest map (includes 'binary')."""
        expected = {
            "code": {".php", ".js", ".py", ".sh", ".bat", ".sql", ".inc", ".module"},
            "markup": {".html", ".htm", ".xml", ".twig", ".tpl", ".svg"},
            "style": {".css", ".scss", ".less", ".sass"},
    ...
    def test_severity_order_matches_both_orderings(self) -> None:
        """`SEVERITY_ORDER` agrees with both descending- and ascending-keyed dicts."""
        descending = {"CRITICAL": 4, "HIGH": 3, "MEDIUM": 2, "LOW": 1, "INFO": 0}
        ascending = {"INFO": 0, "LOW": 1, "MEDIUM": 2, "HIGH": 3, "CRITICAL": 4}
        assert constants.SEVERITY_ORDER == descending
        assert constants.SEVERITY_ORDER == ascending

    def test_severity_cvss_values(self) -> None:
        """`SEVERITY_CVSS` exposes the expected CVSS midpoint per severity bucket."""
        expected = {
            "CRITICAL": 9.5,
            "HIGH": 8.0,
            "MEDIUM": 5.5,
    ...
    def test_skip_dirs_superset_of_intersection(self) -> None:
        """Canonical set includes the directories every caller skips."""
    ...
    def test_scannable_superset_of_sensitive_data(self) -> None:
        """21 extensions used by the sensitive-data scanner are all scannable."""
        required = frozenset({
            ".php", ".inc", ".js", ".json", ".yml", ".yaml", ".xml", ".sql",
            ".txt", ".csv", ".html", ".htm", ".conf", ".ini", ".env", ".log",
            ".md", ".htaccess", ".user.ini", ".py", ".sh",
        })
        assert required <= constants.get_scannable_extensions()

    def test_scannable_superset_of_security_scanner(self) -> None:
        """11 core extensions used by the security scanner are all scannable."""
        required = frozenset({
