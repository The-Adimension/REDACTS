"""Critical configuration file integrity checks.
...
class ConfigIntegrityStep:
    ...
    def _check_database_php(self, root: Path) -> dict[str, Any]:
        ...
        # database.php is the canonical credential-bearing file in a
        # REDCap install; under no circumstances should a raw matched
        # substring or its rendered message survive into a report.
        for v in violations:
            if "matched" in v:
                v["matched"] = redact_preview(v["matched"])
            if "message" in v:
                v["message"] = redact_preview(v["message"])
        sha = sha256(db_php)
    ...
    def _check_user_ini_files(self, root: Path) -> list[dict[str, Any]]:
        ...
        for user_ini in root.rglob(".user.ini"):
            ...
            try:
                content = _read_text_capped(user_ini)
            except Exception as exc:
                results.append({"path": rel, "error": str(exc)})
                continue
            ...
            results.append(
                {
                    ...
                    "content_preview": redact_preview(content[:500]),
