"""Baseline validator \u2014 known-good ZIP vs. installed tree.

The cheapest way to find tampering in a PHP application is to compare
SHA-256 sums against the vendor archive. Files whose hashes match the
official REDCap ZIP extract are passed over by every downstream
scanner; only the *delta set* (added, removed, or modified) goes into
deep analysis.

This is not a substitute for behavioural detection. The InfiniteRed /
Vanderbilt advisories that prompted REDACTS \u2014 the cases where attackers
backdoored ``index.php`` and the ``Classes/`` tree on REDCap installs
that had been left unpatched for years \u2014 turned up because someone
finally diffed a customer's webroot against the upstream archive. A
hash diff is the first thing a forensic responder runs, so REDACTS does
it first too.

Phases

1. Build SHA-256 manifest from the reference ZIP extract.
2. Structural diff: enumerate added / removed / matched files.
3. Integrity check: hash-compare matched files. Equal hash \u21d2 CLEAN
   (skipped). Mismatch \u21d2 MODIFIED, queued for deep analysis.
4. (Downstream) Security regex, tree-sitter, IoC, and external scanners
   run only against the delta set produced by :py:meth:`diff`.

Limitation worth stating: a baseline tells you a file changed, not who
changed it or why. Custom hooks and locally-developed plugins will
register as MODIFIED and require human triage \u2014 see ``case.toml`` for
...
class StructuralDiffResult:
    """Structural and hash diff between reference and target."""
...
class BaselineValidationReport:
    """Compatibility wrapper. Prefer ``StructuralDiffResult``."""
...
class BaselineValidator:
    ...
    * :py:meth:`diff` - 4-phase audit diffing.
    * :py:meth:`validate_installation` - full-report approach.
    """
    ...
    _SKIP_DIRS = {"__pycache__", ".git", ".svn", "node_modules", "vendor", ".tox"}

    def __init__(self) -> None:
        self.baselines_cache: dict[str, ChecksumBaseline] = {}

    # Preferred entry point for audit mode.
    ...
    def diff(
        ...
        Builds SHA-256 manifests, compares tree structure, then hashes
        matched files to isolate the delta set.
        """
        result = StructuralDiffResult(version=version, reference_source="local_archive")

        # Hash both trees.
        logger.info("Hashing reference tree: %s", reference_root)
        ref_hashes = self._hash_tree(reference_root)
        result.reference_file_count = len(ref_hashes)

        logger.info("Hashing target tree: %s", target_root)
        ...
        # Integrity check on matched files.
    ...
    def validate_installation(
        ...
        redcap_root: Path,
        version: str,
        baseline: ChecksumBaseline | None = None,
    ...
    def _hash_tree(self, root: Path) -> dict[str, str]:
        ...
        Delegates to :func:`core.hashing.hash_tree`.
        """
    ...
    def _fetch_or_cache_baseline(self, version: str) -> ChecksumBaseline | None:
