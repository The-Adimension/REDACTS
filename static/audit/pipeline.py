"""Differential audit pipeline - scan only what differs from a known baseline.

Driven by ``[inputs.target]`` and ``[inputs.reference]`` in ``case.toml``.
Invoked indirectly via ``python main.py scan``; this module is not a
separate CLI.

Workflow:

    1. SHA-256 manifests for the reference and target trees.
    2. Structural diff - added / removed / matched files.
    3. Integrity check - hash-compare matched files. Identical files
    short-circuit to CLEAN; mismatches feed deep analysis.
    4. Deep analysis scoped to the delta set (added + modified).

Why differential: stock REDCap ships ~5,000 files. Running the
regex/AST scanners over all of them produces a long tail of
findings on Vanderbilt-shipped code that is identical across every
deployment - the analyst has to triage findings that belong to the
upstream project, not to this site. The hash comparison removes those
before they reach deep analysis.

Known limits:

    * The reference tree must be trustworthy. We hash the reference
      and compare against the published REDCap tarball checksum when
      available; if the analyst supplies a self-prepared reference,
      the audit's integrity claim only goes back as far as that
      reference.
    * A file that is byte-identical to baseline but operates as a
      backdoor through a config-driven path elsewhere will not be
      picked up by the hash comparison. The non-differential scan
      path remains available for that case.
    * Renames look like add+remove. The structural comparison does not currently
      attempt rename detection; a renamed-but-unchanged file shows
    up in the delta set and is re-analysed - extra work,
...
from ..analyze.investigator import Investigator, InvestigationReport
from ..collect.loaders import detect_loader, detect_redcap_root
from threat_base.session import KnowledgeSession
...
class AuditResult:
    ...
    investigation_report_obj: Any | None = field(default=None, repr=False)
...
class AuditPipeline:
    ...
    def __init__(
        ...
        *,
        validator: BaselineValidator | None = None,
        knowledge: KnowledgeSession | None = None,
    ) -> None:
        self.config = config or REDACTSConfig()
        self.validator = validator or BaselineValidator()
        self._knowledge = knowledge or KnowledgeSession()
    ...
    def run(
        ...
        try:
            ...
            # File-count safety cap
            max_files = self.config.analysis.max_total_files
            if result.target_file_count > max_files:
                msg = (
                    f"Target contains {result.target_file_count:,} files, "
                    f"exceeding max_total_files={max_files:,}. "
                    f"Increase analysis.max_total_files in config or use a "
                    f"smaller target directory."
                )
                result.errors.append(msg)
                result.success = False
                result.duration_seconds = time.monotonic() - t0
                logger.error("Audit pipeline aborted: %s", msg)
                return result
        ...
        try:
            ...
            # Deep forensic analysis (delta only).
            _progress("deep_analysis", 0.30)
            delta_set = diff.delta_files

            investigator = Investigator(
                self.config,
                ioc_db=self._knowledge.ioc_db,
                attack_db=self._knowledge.attack_vector_db,
                sensitive_scanner=self._knowledge.sensitive_scanner,
            )
    ...
    def _render_markdown(self, result: AuditResult) -> str:
        ...
        a(f"*REDACTS v{_REDACTS_VERSION} - Audit Mode - {result.timestamp}*")
        return "\n".join(lines)
