...
from .loaders import LoaderError, detect_loader, detect_redcap_root
from .repomix import RepomixRunner
from ..core import REDACTSConfig
from ..core.constants import VERSION
from ..core.sandbox import IntegrityChecker
...
class EvidenceMetadata:
    ...
    # Identity
    label: str = ""
    evidence_id: str = ""  # Unique ID assigned at collection time
...
class EvidenceCollector:
    ...
    def collect(
        ...
        # Heartbeat callback: log progress to the CLI at INFO so the log
        # file advances during the long Magika-classification loop.
        last_log_time = [time.time()]

        def _manifest_progress(done: int, total: int, message: str) -> None:
            now = time.time()
            if now - last_log_time[0] >= 5.0 or done == total:
                logger.info(
                    "[2/%d] manifest progress: %d/%d files",
                    total_steps, done, total,
                )
                last_log_time[0] = now
        ...
        # Write evidence package to disk.
    ...
    def _load_source(
        ...
        Catches :class:`LoaderError` from the loader hierarchy and
        :class:`OSError` / :class:`ValueError` from filesystem and URL
        parsing. Any other exception type is treated as a programmer
        error and re-raised so it surfaces in the test suite rather
        than being silently recorded in ``package.errors``.
        """
        try:
            ...
            return root
        except (LoaderError, OSError, ValueError) as e:
