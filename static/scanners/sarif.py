...
Provides canonical free-functions used by :class:`SemgrepAdapter` and
:class:`TrivyAdapter` to extract result objects and to count findings
by severity, so each adapter does not need its own copy.
...
* :func:`extract_sarif_results` returns ``list[dict[str, Any]]``.
* :func:`count_by_severity` returns ``dict[str, int]`` with
  severity *.value* keys.
...
class LocationStrategy(Protocol):
    ...
    def __call__(
        ...
        raise RuntimeError("LocationExtractor.__call__ must be implemented by subclasses")
...
# --- extract_sarif_results (identical on both adapters) ----
...
# --- count_by_severity (identical on both adapters) --------
