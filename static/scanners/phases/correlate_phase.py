...
context : OrchestratorContext
    Mutated in place.  Findings already in
    ``context.collection.findings`` are re-tagged with corroboration
    metadata; nothing is removed.

Notes
Trivy -> Semgrep synergy is best-effort and string-based.  Component
matching uses the dependency name as it appears in the Trivy CVE
description; this gives false positives when a component name shares
a substring with unrelated tokens (e.g. ``log4j-core`` matching
``catalog4java``).  We accept that here because the cost of a missed
correlation is higher than a noisy one.
"""
...
from ...core.findings import Confidence, FindingSource
from ..phase_protocol import OrchestratorContext, PhaseResult

logger = logging.getLogger(__name__)

__all__ = ["CorrelatePhase"]


class CorrelatePhase:
    """Cross-tool correlation and Magika enrichment."""

    name: str = "correlate"

    def execute(self, context: OrchestratorContext) -> PhaseResult:
        logger.info("Cross-tool correlation...")
