"""Structural finding enrichment via tree-sitter.

The detection pipeline is line-oriented.  Investigators need callsite
context: which function, which class, how complex.  This phase parses
each PHP file once and back-fills ``finding.evidence`` with the
enclosing scope, mirroring the ``radare2 afl`` style of structural
metadata.

Honest limit: enrichment fails open.  When tree-sitter cannot parse a
file (truncated source, unsupported syntax) the finding survives
without context - we never silently drop a finding because of an
analysis failure.
"""
...
from ...core.findings import UnifiedFinding
from ..phase_protocol import OrchestratorContext, PhaseResult

logger = logging.getLogger(__name__)

__all__ = ["TreeSitterPhase"]


class TreeSitterPhase:
    """Enrich findings with structural context from tree-sitter.
    ...
    def execute(self, context: OrchestratorContext) -> PhaseResult:
        ...
        logger.info("tree-sitter enrichment...")
...
def _find_enclosing_context(ast: Any, line: int) -> dict[str, Any] | None:
