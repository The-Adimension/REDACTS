"""Trivy dependency CVE and secret scanning.

Trivy CVE results are filtered to the delta set: a CVE on a stock
``composer.lock`` that is identical between reference and target is a
pre-existing bug, not a compromise indicator.  Treating it as such
would flood reports during forensic triage.
"""
...
class TrivyPhase:
    """Run Trivy for dependency CVE scanning and secret detection."""
    ...
    def execute(self, context: OrchestratorContext) -> PhaseResult:
        ...
        try:
            ...
            if context.config.generate_sbom and result.success:
                ...
                sbom_dir.mkdir(parents=True, exist_ok=True, mode=0o700)
                sbom_path = Path(
