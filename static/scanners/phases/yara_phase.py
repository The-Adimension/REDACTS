"""YARA webshell and backdoor detection.

YARA + Magika is the highest-confidence corroboration available in the
pipeline: a YARA rule fires on content whose Magika type also disagrees
with the file extension (e.g. PHP detected inside ``avatar.jpg``).
That combination is treated as ``Confidence.CONFIRMED`` because both
signals are independent --- one inspects bytes, the other inspects
syntax.
"""
...
from ...core.findings import Confidence, SeverityLevel, normalize_yara_match
...
class YaraPhase:
    """Run YARA with community rules for webshell/backdoor detection."""
