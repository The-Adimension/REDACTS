"""Magika adapter - content-type detection independent of file extension.

Magika is Google's open-source content-type classifier
(https://github.com/google/magika, paper: arXiv:2409.13768). It runs
a ~2 MB CNN trained on ~100 M files across 200+ content types and
classifies a buffer in single-digit milliseconds on CPU.

We rely on Magika because the InfiniteRed pattern (and most webshell
families) deliberately mismatch extension and content - PHP renamed
``.jpg``, SQLite dropped as ``cache.tmp``, polyglots that parse as
both GIF and PHP. Extension-based MIME guessing
(``mimetypes.guess_type``) cannot see those.

Known limits (per the Magika model card):

    * Accuracy is reported around 99 % on the model's evaluation set.
      The remaining ~1 % is concentrated in short files (< 16 bytes,
      where there is little signal), heavily commented source where
      the comments outweigh the code, and adversarial polyglots.
      REDACTS treats a low-confidence Magika verdict as a finding to
      review, not as ground truth.
    * The model is frozen at install time. Content types added to
      Magika after our pinned version (see ``requirements.txt``) will
      surface as ``unknown`` rather than the new label.
    * A correct content type is not the same as a safe file. Magika
      is the routing input, not the verdict.

Magika is a hard dependency; missing or broken loads raise rather
than silently degrading the routing.

Integration points:

* :class:`static.collect.manifest.ManifestBuilder` - Tier 1 typing
* :class:`static.scanners.phases.MagikaPhase` - orchestrator routing
* :class:`static.analyze.investigator.Investigator` - mismatch findings
"""
...
# is a strong signal for data exfiltration a la INFINITERED.
...
def _classify_mismatch(
    ...
    Returns:
        (severity, detail)  where severity  in  {"none","info","suspicious","critical"}
    """
        ...
        for idx, fpath in enumerate(files, 1):
            ...
            if progress_callback and idx % 100 == 0:
                try:
                    ...
                    logger.debug("Magika progress callback failed", exc_info=True)
