"""Trivy adapter - dependency CVE and secret scanning.

Trivy (https://github.com/aquasecurity/trivy) scans for:

    * Known CVEs in Composer dependencies (``composer.lock``).
    * Leaked secrets in tracked files (high-entropy strings + a
      curated rule set; see Trivy's secret-scanning docs).
    * SBOM generation for supply-chain visibility.
    * Dockerfile / IaC misconfigurations on the runtime images.

What Trivy does not do: it is version-matching plus pattern matching.
It cannot reason about PHP semantics, so it will not catch
``eval(base64_decode(...))`` and similar runtime payloads. Pair it
with the Semgrep and YARA phases.

Known limits:

    * CVE coverage is only as fresh as the local feed. Trivy refreshes
      the database on the first scan of each day by default; for
      air-gapped runs, schedule ``trivy image --download-db-only``
      and pass ``--skip-db-update`` to the adapter.
    * Composer-only. Other PHP package sources (PEAR, raw drop-in
      libraries) bypass the dependency scan.

Results are read from Trivy's native SARIF 2.1.0 output and
re-mapped into :class:`UnifiedFinding`. Trivy is a hard dependency;
a missing binary raises with install instructions.
"""
...
from .external import DEFAULT_TOOL_TIMEOUT, ExternalToolAdapter, ExternalToolResult, _resolve_venv_tool
from .sarif import (
    count_by_severity,
...
class TrivyAdapter(ExternalToolAdapter):
    ...
    Provides three capabilities the rest of the pipeline does not
    cover: Composer lockfile CVE scanning, secret detection alongside
    :mod:`threat_base.sensitive_data`, and SBOM generation for
    supply-chain review.
    """
    ...
    def run(
        ...
        return ExternalToolResult(
            ...
            available=True,
            # Trivy uses ``--exit-code`` to signal
            # *vulnerabilities found* (commonly 1 or 3 depending on
            # configuration). Exit code 3 in particular is "issues
            # found, scan succeeded" - that is the *expected* outcome
            # for a security tool, not a failure. Treat it as success
            # so the orchestrator does not discard a valid SARIF
            # payload. Accept both 0 and 3 as success; any other
            # non-zero code remains a genuine tool failure.
            success=rc in (0, 3),
