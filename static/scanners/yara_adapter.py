...
Runs the ``yara`` binary against the target tree using a merged rule
set: REDACTS-bundled rules plus optional community sources fetched on
first use:

    * PHP-Malware-Finder (``php.yar``) - LGPL-3.0 webshell rules
      (https://github.com/jvoisin/php-malware-finder)
    * Neo23x0/signature-base - generic webshell + exploit signatures
      (https://github.com/Neo23x0/signature-base)
    * Any user-supplied ``*.yar`` files under the configured rules dir

Known limits:

    * YARA is a signature engine. Obfuscated, encrypted, or never-
      before-seen payloads will not match. Pair with the Magika and
      Semgrep phases for coverage.
    * Community rule quality varies. False-positive prone rules are
      filtered out via the curated ``allowlist`` in
      ``community_rules.py``; new rules go through that list before
      reaching production scans.
    * Network fetches for community rules go through
      :func:`reject_ssrf_target` and :func:`enforce_https` so the
      first-use bootstrap cannot be redirected to an attacker host.

Matches are normalised into :class:`UnifiedFinding` so the orchestrator
can correlate them with Semgrep and Trivy hits on the same file.
"""
...
class YaraAdapter(ExternalToolAdapter):
    ...
    def _default_rules_dir() -> Path:
        """Return cache dir for community rules.

        Resolved against the active :class:`FrozenCaseContract` via
        :func:`static.core.paths.cache_dir`. There is no environment
        override; ``REDACTS_DATA_DIR`` is forbidden.
        """
        from static.core.paths import cache_dir
        rules = cache_dir() / "yara_rules"
    ...
    def _sanitize_php_malware_finder(rule_file: Path) -> None:
        ...
        if text != original:
            ...
            logger.debug("php-malware-finder rules required no sanitization - patterns may have changed upstream")
