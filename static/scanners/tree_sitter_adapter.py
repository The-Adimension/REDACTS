"""tree-sitter PHP analyser - replaces the regex-based PHP parser.

The regex parser this superseded confused matches inside strings and
comments with real declarations, miscounted braces in heredocs, and
attributed methods to the wrong class. Switching to tree-sitter
(https://tree-sitter.github.io/tree-sitter/) and the official
``tree-sitter-php`` grammar removes that whole error class.

The output dataclasses (:class:`PHPFileAST` and friends) match the
shape the previous parser returned so downstream consumers - diff,
manifest, reporting - did not have to change.

Known limits:

    * Dynamically constructed code (``eval``, ``create_function``,
      base64-decoded ``include``) is opaque to any AST parser. Those
      paths are caught only by the runtime DAST stage or by Magika's
      content-type check on the dropped file.
    * The grammar tracks PHP through 8.x. Files using newer syntax
      may parse with ERROR nodes; we surface those rather than skip
      the file silently.
    * Cyclomatic complexity counts control-flow nodes only. It does
      not weight nesting depth - a deeply nested simple function and
      a flat complex one can receive the same score.

``tree-sitter`` + ``tree-sitter-php`` are required dependencies; the
module raises :class:`ImportError` with install instructions rather
than running with a degraded parser.
"""
