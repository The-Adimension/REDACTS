...
class OrchestratorContext:
    ...
    max_file_size_mb: int = 50  # Propagated from AnalysisConfig for phase use
    ...
    # Record phase exceptions for workflow-level exit handling.
    # Each entry is ``(phase_name, exception_repr)``.
    phase_failures: list[tuple[str, str]] = field(default_factory=list)
...
class ScanPhase(Protocol):
    ...
    def execute(self, context: OrchestratorContext) -> PhaseResult:
        """Run this scan phase."""
        raise RuntimeError("ScanPhase.execute must be implemented by subclasses")
