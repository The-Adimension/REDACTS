...
To update, run ``python main.py update cwe`` manually.
That script requires explicit user confirmation at every step.

CWE(TM) content is derived from the Common Weakness Enumeration, which is
...
Copyright (c) 2006-2026, The MITRE Corporation.  CWE, CWSS, CWRAF, and
...
def cwe_source_url() -> str:
    """Return the CWE CSV source URL - sourced from the active contract.

    See :mod:`threat_base.sources`.
    """
...
def _compute_file_sha256(path: Path) -> str:
    """Compute SHA-256 of a file using the canonical hashing helper."""
    from static.core.hashing import compute_single_hash

    return compute_single_hash(path, algorithm="sha256")
...
def _load_csv(csv_path: Path) -> dict[str, CweEntry]:
    ...
    if not csv_path.is_file():
        logger.warning("CWE CSV not found: %s", csv_path)
        return {}
    ...
    def __init__(self, *, data_dir: Path | None = None) -> None:
        ...
        if self._integrity.status == "VERIFIED":
            ...
            logger.warning(
                ...
                "Run 'python main.py update cwe' to download.",
                self._csv_path,
            )
            ...
            raise ValueError(
                ...
                f"Run 'python main.py update cwe' to re-download."
            )
